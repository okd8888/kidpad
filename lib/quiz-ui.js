 ...
 *               hintTip  = 鷹架出現時要說的話，預設「看看下面的提示喔」
 *               say   = 題目出現時要唸的英文（需要 onSpeak）
 *               sayZh = 題目出現時要唸的中文（畫面上的題目直接唸出來）
 *   onResult({ question, firstTry, wrongCount }) 每題完成時呼叫，可用來記錄個別能力
...
export function createQuizView(container, opts) {
  const {
    roundSize = 6, makeQuestion, onResult, onFinish, doneLabel, onDone, buddy, onSpeak,
  ...
  let hintTip = '';     // 鷹架出現時要說的話
  let currentQuestion = null;
  let lastPrompt = null; // 上一題長什麼樣，用來避免連續出一樣的題目
  let timer = null;
  let guardTimer = null;
  let idleTimer = null;
  ...
  function showQuestion() {
    ...
    // 跟上一題一模一樣就重抽，最多試 8 次（題庫太小就認了，免得卡住）
    let q = makeQuestion(index, { combo, correctCount });
    for (let i = 0; i < 8 && q.promptHtml === lastPrompt; i++) {
      q = makeQuestion(index, { combo, correctCount });
    }
    lastPrompt = q.promptHtml;
    currentQuestion = q;

    el.prompt.innerHTML = q.promptHtml;

    hintHtml = q.hintHtml || null;
    hintTip = q.hintTip || '看看下面的提示喔';
    el.hint.hidden = true;
    el.hint.innerHTML = '';
  ...
  function answer(btn, isCorrect) {
    ...
    onResult?.({ question: currentQuestion, firstTry, wrongCount: wrongHere });
    renderHead();

    timer = setTimeout(() => { timer = null; advance(); }, NEXT_DELAY);
  }
  ...
  return {
    start,
    /** 模組被切走：答對後正在等著跳下一題的話，直接跳完，
        不然切回來會卡在「已經答對但畫面沒換」的狀態，按什麼都沒反應。 */
    pause() {
      clearTimeout(guardTimer);
      stopIdle();
      speech.stop();
      // 這行要放最後：advance() 會排一個新的解鎖計時器，先清才不會把它一起清掉
      if (timer) { clearTimeout(timer); timer = null; advance(); }
    },
  };
}
