 ...
 * @param {object} opts
 *   roundSize   一輪幾題
 *   makeQuestion(index, ctx) ctx = { combo, correctCount }，回傳 { promptHtml, options:[{text, correct}] }
 *   onFinish({ correct, total }) 一輪結束時呼叫，回傳值當作這一輪拿到的星星數
 *   doneLabel   結算畫面的按鈕文字（預設「再玩一次」）
 *   onDone()    按下結算按鈕時做什麼（預設重新開一輪）
 *   buddy       陪在旁邊的小動物 emoji，答對會跳、答錯會歪頭
 *   onSpeak(t)  題目裡按下 [data-say] 的喇叭時要唸什麼；沒給就不處理
 */
export function createQuizView(container, opts) {
  const { roundSize = 6, makeQuestion, onFinish, doneLabel, onDone, buddy, onSpeak } = opts;
  ...
  container.innerHTML = `
    <div class="quiz">
      ...
      ${buddy ? `<div class="buddy" id="buddy">${buddy}</div>` : ''}
      ...
      <div class="reward" id="quizReward" hidden>
        <div class="reward-inner">
          ...
          <button class="kid-btn mint" id="rewardAgain" type="button"></button>
        </div>
      </div>
    </div>
  `;
  ...
  // 題目裡的喇叭按鈕
  if (onSpeak) {
    container.querySelector('#quizPrompt').addEventListener('click', e => {
      const b = e.target.closest('[data-say]');
      if (b) onSpeak(b.dataset.say);
    });
  }

  const doneBtn = container.querySelector('#rewardAgain');
  ...
  return {
    start,
    /** 模組被切走時呼叫：停掉排程中的跳題 */
    pause() { clearTimeout(timer); clearTimeout(guardTimer); },
  };
}
