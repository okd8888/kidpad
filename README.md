...
點左邊哪個功能，右邊就換成對應的畫面。目前完成的是**中文筆劃練習**和**算術練習**。
...
10 以內加減（Level 2），一輪 6 題。起點設在這一級是因為小朋友 10 以內加減已經會了，
完整的內容規劃見 [docs/CONTENT-PLAN.md](docs/CONTENT-PLAN.md)。

| 功能 | 說明 |
|---|---|
| 四種題型 | 圖像加法（🍎🍎🍎＋🍎🍎）、圖像減法（吃掉的變半透明）、純算式、湊十（7＋□＝10） |
| 選項 | 3 選 1 的大按鈕，答案附近的數字當干擾選項 |
| 答對 | 按鈕變綠、**星星粒子噴出來**、音效，0.8 秒後自動跳下一題 |
| 連對 | 連對 2 題冒出「連對 2！」，3 題以上換成 🔥 |
| 答錯 | 只有淡出變灰＋輕晃＋柔和音，可以一直重選；不變紅、不打叉、不計時、不扣分 |
| 星星 | 一輪 6 題結束給 1 顆，全對給 2 顆，累積在左側選單的按鈕上 |

> 關卡地圖、升降級、20 以內進退位排在 S2；貼紙簿與獎章排在 S5。

### 英文練習

目前是「準備中」的佔位模組（排在 S3），之後照下面的方式補上內容就好。
