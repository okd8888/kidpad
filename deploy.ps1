...
if ($LASTEXITCODE -ne 0) {
  Line '[X] 這個資料夾還不是 git repository。請先執行一次：'
  Line '    git init'
  Line '    git branch -M main'
  Line '    git remote add origin https://github.com/你的帳號/kidpad.git'
  Bye
}
...
if ($dirty) {
  ...
  # 訊息可以用參數帶入：deploy.bat "新增算術練習"
  $msg = if ($args.Count -gt 0) { $args -join ' ' } else { '' }
  if (-not $msg) { $msg = Read-Host '請輸入這次改了什麼（直接按 Enter 用「更新練習台」）' }
  if (-not $msg) { $msg = '更新練習台' }
